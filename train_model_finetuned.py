"""
Fine-Tuned Time-to-Failure Prediction Model Training Script
Includes hyperparameter tuning, feature engineering, and cross-validation
"""
...
print("=" * 70)
print("Fine-Tuned Time-to-Failure Prediction Model Training")
print("=" * 70)
...
if 'month' in X_fe.columns:
    X_fe['is_peak_season'] = X_fe['month'].isin([6, 7, 8, 12, 1]).astype(int)  # Summer and winter peaks
...
print("\n[7/7] Training and fine-tuning models...")
print("=" * 70)
...
# Model 1: Fine-tuned Random Forest
print("\n" + "-" * 70)
print("Fine-tuning Random Forest Regressor...")
print("-" * 70)
...
rf_search = RandomizedSearchCV(
    rf_base, rf_param_dist, n_iter=30, cv=5, 
...
print("\n" + "-" * 70)
print("Fine-tuning XGBoost Regressor...")
print("-" * 70)
...
xgb_search = RandomizedSearchCV(
    xgb_base, xgb_param_dist, n_iter=40, cv=5,
    scoring='neg_mean_absolute_error', n_jobs=-1,
    random_state=42, verbose=1
)
...
# Model 3: Gradient Boosting (additional ensemble)
print("\n" + "-" * 70)
print("Fine-tuning Gradient Boosting Regressor...")
print("-" * 70)
...
gb_search = RandomizedSearchCV(
    gb_base, gb_param_dist, n_iter=25, cv=5,
    scoring='neg_mean_absolute_error', n_jobs=-1,
    random_state=42, verbose=1
)
...
models['GradientBoosting_Tuned'] = gb_model
...
print(f"  Train MAE: {gb_mae_train:.2f} days")
print(f"  Test MAE: {gb_mae_test:.2f} days")
print(f"  CV MAE: {gb_cv_mae:.2f} days")
print(f"  Train RMSE: {gb_rmse_train:.2f} days")
print(f"  Test RMSE: {gb_rmse_test:.2f} days")
...
feature_info = {
    ...
    'best_params': best_params[best_model_name.split('_')[0]] if best_model_name.split('_')[0] in best_params else None
}
